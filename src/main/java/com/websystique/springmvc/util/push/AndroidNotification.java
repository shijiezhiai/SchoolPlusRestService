...
public abstract class AndroidNotification extends UmengNotification {
	...
	protected static final HashSet<String> PAYLOAD_KEYS = new HashSet<>(
	        Arrays.asList(new String[]{
	            "display_type"}));
	
	// Keys can be set in the body level
	protected static final HashSet<String> BODY_KEYS =
			new HashSet<>(Arrays.asList(new String[] {
			"ticker", "title", "text", "builder_id", "icon", "largeIcon", "img",
            "play_vibrate", "play_lights", "play_sound", "sound", "after_open",
            "url", "activity", "custom"}));
	...
	public enum DisplayType{
		...
		MESSAGE { //消息:消息送达到用户设备后，消息内容透传给应用自身进行解析处理。
			public String getValue() {
				return "message";
			}
		};
	...
	// Set key/value in the rootJson.
	// For the keys that can be set, please see ROOT_KEYS, PAYLOAD_KEYS,
